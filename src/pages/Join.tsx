...
import { User, ArrowLeft, CheckCircle2, Loader2, Clock, Users } from 'lucide-react';
import { motion } from 'motion/react';
import toast from 'react-hot-toast';
import { useQueueCount, useAverageServiceTime } from '../hooks/useQueue';
...
export default function Join() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!customerId) {
        throw new Error('Não foi possível identificar o cliente. Por favor, tente novamente.');
      }
      ...
      for (let i = 0; i < 4; i++) {
        nextCode += allChars.charAt(Math.floor(Math.random() * allChars.length));
      }
      ...
      nextCode = nextCode.split('').sort(() => Math.random() - 0.5).join('');
      ...
      if (!queueEntry) {
        throw new Error('Falha ao confirmar entrada na fila. Por favor, tente novamente.');
      }
      ...
      const errorMessage = error?.message || 'Falha ao entrar na fila. Por favor, tente novamente.';
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-8 bg-neutral-50 dark:bg-neutral-950">
      ...
      >
        ...
        <div className="space-y-2 text-center">
          <h1 className="text-3xl font-bold text-neutral-900 dark:text-white">Quase lá!</h1>
          <p className="text-neutral-500 dark:text-neutral-400">Confirme seus dados para entrar na fila.</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl bg-white p-4 text-center border border-neutral-200 shadow-sm dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <p className="text-xs font-bold uppercase text-neutral-500 dark:text-neutral-500">Sua Posição</p>
            <p className="text-2xl font-black text-neutral-900 dark:text-white">{queueCount + 1}º</p>
          </div>
          <div className="rounded-xl bg-white p-4 text-center border border-neutral-200 shadow-sm dark:bg-neutral-900 dark:border-neutral-800">
            <Clock className="mx-auto mb-2 h-6 w-6 text-emerald-600" />
            <p className="text-xs font-bold uppercase text-neutral-500 dark:text-neutral-500">Espera Estimada</p>
        ...
        <form onSubmit={handleJoin} className="space-y-6 rounded-3xl bg-white p-8 shadow-xl shadow-neutral-200/50 dark:bg-neutral-900 dark:shadow-none dark:border dark:border-neutral-800">
          <div className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-semibold text-neutral-700 dark:text-neutral-300">Número de Telefone</label>
            ...
            <div>
              <label className="mb-2 block text-sm font-semibold text-neutral-700 dark:text-neutral-300">Seu Nome</label>
